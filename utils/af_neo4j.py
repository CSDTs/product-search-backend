...
class App:
    ...
    def add_offering(self, maker=None, craft_id=None,
                     url=None, image=None, product_name=None):
        with self.driver.session() as session:
            # Write transactions allow the driver to handle retries and transient errors
            result = session.write_transaction(
                self._add_offering, maker, craft_id, url, image, product_name)
            for record in result:
                print("Created artisan: {p1} for {img}".format(
                    p1=record['p1'], img=image))
    ...
    def _add_offering(tx=None,
        ...
        cypher = (
            ...
            "MERGE (img:Image {name: $image }) "
            "MERGE (prod:Product {name: $product_name }) "
            "MERGE (a)-[:MADE]->(cid) "
            "MERGE (cid)-[:HAS_URL]->(url) "
            "MERGE (cid)-[:HAS_IMG]->(img) "
            "MERGE (cid)-[:INSTANCE_OF]->(prod) "
        )
        result = tx.run(cypher, maker=maker, craft_id=craft_id, url=url, image=image, product_name=product_name)
        try:
            return [record
                    for record in result]
    ...
    def check_database(self, what='Artisan'):
        with self.driver.session() as session:
            # Write transactions allow the driver to handle retries and transient errors
            result = session.read_transaction(
                self._check_database, what=what)

            return result                

    @staticmethod
    def _check_database(tx=None, what='Artisan'):
        cypher = (
            f"MATCH (n:{what}) RETURN n"
        )
        result = tx.run(cypher)

        return [a_result.data()['n']['name'] for a_result in result]

    def query_database(self, cypher=None):
        with self.driver.session() as session:
            # Write transactions allow the driver to handle retries and transient errors
            result = None
            if cypher is not None:
                result = session.read_transaction(
                    self._query_database, cypher=cypher)

            return result

    @staticmethod
    def _query_database(tx=None, cypher=None):
        result = tx.run(cypher)
...
def write_to_graph_db(df):
    ...
    for THE_ROW_INDEX in range(len(df)):
        ...
        print(f"{df.loc[THE_ROW_INDEX].image} adding offering w that ")
        app.add_offering(
            df.loc[THE_ROW_INDEX].artisan,
            craft_id=hash_of_offering,
            url=df.loc[THE_ROW_INDEX].url,
            image=df.loc[THE_ROW_INDEX].image,
